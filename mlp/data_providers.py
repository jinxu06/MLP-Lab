...
class MNISTDenoisingAutoencoderDataProvider(MNISTDataProvider):
    ...
    def __init__(self, noise_pct, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None):
        super(MNISTDenoisingAutoencoderDataProvider, self).__init__(which_set, batch_size, max_num_batches,
                 shuffle_order, rng)
    ...
    def next(self):
        ...
        mask = np.random.binomial(1, 1-self.noise_pct, inputs_batch.shape)
        return inputs_batch*mask, inputs_batch


class MNISTCNNDataProvider(MNISTDataProvider):

    def next(self):
        inputs_batch, targets_batch = super(
            MNISTCNNDataProvider, self).next()
        inputs_batch = inputs_batch.reshape((self.batch_size, 1, 28, 28))
        return inputs_batch, targets_batch

class MNISTCNNAutoencoderDataProvider(MNISTAutoencoderDataProvider):

    def next(self):
        inputs_batch, targets_batch = super(
            MNISTCNNAutoencoderDataProvider, self).next()
        inputs_batch = inputs_batch.reshape((self.batch_size, 1, 28, 28))
        targets_batch = targets_batch.reshape((self.batch_size, 1, 28, 28))
        return inputs_batch, targets_batch

class MNISTCNNDenoisingAutoencoderDataProvider(MNISTDenoisingAutoencoderDataProvider):

    def next(self):
        inputs_batch, targets_batch = super(
            MNISTCNNDenoisingAutoencoderDataProvider, self).next()
        inputs_batch = inputs_batch.reshape((self.batch_size, 1, 28, 28))
        targets_batch = targets_batch.reshape((self.batch_size, 1, 28, 28))
        return inputs_batch, targets_batch
...
class MNISTCNNResizeDataProvider(MNISTCNNDataProvider):

    def __init__(self, img_width= 28, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None):
        super(MNISTCNNResizeDataProvider, self).__init__(which_set, batch_size, max_num_batches,
                 shuffle_order, rng)
        self.img_width = img_width

    def next(self):
        inputs_batch, targets_batch = super(
            MNISTCNNResizeDataProvider, self).next()
        inputs_resize = np.zeros_like(inputs_batch)
        for i,inputs in enumerate(inputs_batch):
            inputs = scipy.misc.imresize(inputs[0].reshape((28, 28)),
                 size=(28, self.img_width))
            padding_size = (28 - self.img_width)/2
            inputs = np.pad(inputs, ((0,0),(padding_size, padding_size)), mode='constant')
            inputs_resize[i][0] = inputs
        #inputs_resize = inputs_resize.reshape((self.batch_size, 1, 28, 28))
        return inputs_resize, targets_batch
