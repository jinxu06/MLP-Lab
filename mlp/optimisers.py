...
class Optimiser(object):
    ...
    def do_training_epoch(self):
        ...
        self.activations_record.append(activations)
...
class SOptimiser(Optimiser):
    ...
    def do_training_epoch(self):
        ...
        for val_inputs_batch, val_targets_batch in self.train_dataset:
            min_error = 1000000
            chosen = 0
            for i in range(3):
                train_inputs_batch, train_targets_batch = self.train_dataset.next()
         
                activations = self.model.fprop(train_inputs_batch)
                grads_wrt_outputs = self.error.grad(activations[-1], train_targets_batch)
                grads_wrt_params = self.model.grads_wrt_params(
                    activations, grads_wrt_outputs)
                self.learning_rule.update_params(grads_wrt_params)
            
                activations = self.model.fprop(val_inputs_batch)
                e = self.error(activations[-1], val_targets_batch)
                if e < min_error:
                    min_error = e
                    chosen = i+1
                else:
                    self.learning_rule.reverse_params(grads_wrt_params)
